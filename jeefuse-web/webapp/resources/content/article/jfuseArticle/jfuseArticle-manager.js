$(function(){
    ...
    var option = {
        height: h, 
        width: w,
        url: ctx+'/article/jfuseArticle/listOutJson.vhtml',
        colModel: [
    ...
    //variate
    config.inputUrl=ctx+'/article/jfuseArticle/input.vhtml';
    config.editUrl=ctx+'/article/jfuseArticle/edit.vhtml';
    config.deleteOutJsonUrl=ctx+'/article/jfuseArticle/deleteOutJson.vhtml';
    config.deleteOutJsonAllUrl=ctx+'/article/jfuseArticle/deleteAllOutJson.vhtml';
});
var grid;
...
function edit(id){
	...
	var rowDom=grid.getRowDom(id);//获取该ID的行元素.
	var rowIdx=$(rowDom).attr("seq");
	var title=config.baseTitle+'->编辑 (序号:'+rowIdx+')';
	var url=goolov.addUrlParams(config.editUrl,"id="+id);
...
function addRowData(row){
	var indexId=grid.getIndexId();//获取每行唯一标识的属性名,通过row[indexId]获取每行唯一标识的值
	var rowTr=grid.getRowDom(row[indexId]);////获取该ID的行元素.
	if (rowTr.length==0) {
		grid.flexAddRowData(row);		
	}else{
		grid.flexEditRowData(rowDom[0],row);
	}
}
/******del data**********/
function delData(){
...
function exportExcel(){
	...
	var exportExcelFileUrl=ctx+'/article/jfuseArticle/exportExcelFile.vhtml';
...
function importExcelSubmit(){
	$("#importExcelForm").ajaxSubmit({
		 type : 'post',
		 dataType:'html', 
		 url:ctx+'/article/jfuseArticle/importExcelFileProcess.vhtml',
