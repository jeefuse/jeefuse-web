$(function(){
    ...
    var option = {
        height: h, 
        width: w,
        url: ctx+'/system/security/gsysRelUserRole/listOutJson.vhtml',
        colModel: [
		  { display: 'gsysRole',name: 'gsysRole.',width: 120, sortable: false, align: 'center'}
		  ,{ display: 'gsysUser',name: 'gsysUser.',width: 120, sortable: false, align: 'center'}
		],      
...
function onRowDblclick(target){
	...
	editWin.window('setTitle','GsysRelUserRole管理>编辑 (序号:'+rowIdx+')');
	loadingFrame("edit-win-content","edit-win-iframe",ctx+'/system/security/gsysRelUserRole/edit.vhtml?id='+id);
}
function newData(){
	newWin.window({zIndex:$.fn.window.defaults.zIndex++});
	newWin.window('open');
	newWin.window('setTitle','GsysRelUserRole管理>新增');
	loadingFrame("new-win-content","new-win-iframe",ctx+'/system/security/gsysRelUserRole/input.vhtml');
}
function editData(){
	...
	editWin.window('open');
	editWin.window('setTitle','GsysRelUserRole管理>编辑 (序号:'+rowIdx+')');
	loadingFrame("edit-win-content","edit-win-iframe",ctx+'/system/security/gsysRelUserRole/edit.vhtml?id='+ids[0]);
}
function delData(){
	var ids=grid.getCheckedRows();
	var selCount=ids.length;
	...
	goolov.msgbox.confirm('删除确认','您确认删除这'+selCount+'项记录吗?删除后将不可恢复!',function(r){
		if(r){
			$.ajax({
					type : 'post',
					url : ctx+'/system/security/gsysRelUserRole/deleteOutJson.vhtml',
					data : {id:ids.join(',')},
					dataType:'json',
					beforeSend :function() {
...
function delAllData(){
		...
		if(r){
			$.ajax({
					type : 'get',
					url : ctx+'/system/security/gsysRelUserRole/deleteAllOutJson.vhtml',
					dataType:'json',
					beforeSend :function() {
						goolov.growl.waiting('正在删除中!请稍后...');
					},
...
function exportExcel(){
	location.href=ctx+'/system/security/gsysRelUserRole/exportExcelFile.vhtml';
}
var importExcelWin;
function importExcel(){
	...
	importExcelWin.window('setTitle','GsysRelUserRole 导入Excel文件数据');
	loadingFrame("importExcelWinContent","importExcelWinContentIframe",ctx+'/system/security/gsysRelUserRole/importExcelFile.vhtml');
}
